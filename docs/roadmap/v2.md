# v2 Roadmap — Graph-Aware Code Reviewer

## What v2 Is

v2 evolves the current CLI reviewer from diff-aware to **system-aware**. Instead of only seeing changed lines, the reviewer understands the entire codebase — dependency graph, call chains, blast radius, interface contracts.
...
## Build Layers — Follow This Order
...
| Layer       | What                                                  | Status                  |
| ----------- | ----------------------------------------------------- | ----------------------- |
| **Layer 0** | LLM caller + CLI (AgnusAI current)                    | ✅ Exists — do not break |
| **Layer 1** | Indexing pipeline (parser, graph, storage, retriever) | ❌ Not started           |
| **Layer 2** | API server (Fastify, webhooks, SSE)                   | ❌ Not started           |
| **Layer 3** | CI adapters (GitHub Actions + Azure Pipelines)        | ❌ Not started           |
| **Layer 4** | Dashboard + Auth (Next.js, GitHub OAuth + Azure AD)   | ❌ Not started           |
| **Layer 5** | Self-host packaging (Docker Compose)                  | ❌ Not started           |
...
│   │       ├── graph/            # Symbol graph: build, store, traverse
│   │       ├── embeddings/       # Embed symbols, vector search
│   │       ├── indexer/          # Orchestrate full + incremental indexing
│   │       ├── retriever/        # Given a diff, fetch relevant context
│   │       ├── reviewer/         # ← EXISTING LLM caller lives here (Layer 0)
│   │       └── storage/          # DB adapters (SQLite / Postgres, swappable)
│   │
│   ├── api/                      # 
API server
│   │   └── src/
│   │       ├── routes/
│   │       ├── webhooks/         # GitHub + Azure webhook handlers
│   │       ├── auth/             # OAuth handlers
│   │       └── sse/              # Live indexing progress
│   │
│   ├── dashboard/                # Next.js frontend
│   └── shared/                   # Types + Zod schemas shared across packages
│
├── docker-compose.yml
└── .env.example
```

The current `src/` becomes `packages/core/src/reviewer/` — no logic changes, just moved into the monorepo.
...
### Parser — Tree-sitter (not LSP)

Tree-sitter is the right tool for symbol extraction — **not LSP servers**. It's an incremental parsing library that builds a concrete syntax tree (CST), re-parses only changed nodes, and is deterministic (no hallucinated relationships).
...
### Symbol Graph — In-Memory (not Neo4j)
...
```
graph.getCallers(symbolId, hops=2)   → who calls this? (walk inEdges)
graph.getCallees(symbolId, hops=1)   → what does this call? (walk outEdges)
graph.getBlastRadius(symbolIds)      → full impact: files + risk score
graph.removeFile(filePath)           → called before re-parsing a changed file
```

**1-2 hop traversal is the sweet spot.** Beyond 2 hops, token costs balloon with diminishing returns.

### Storage — SQLite default, Postgres for cloud
...
Full index (run once when repo is connected):
  Parse all files → extract symbols → build graph → embed → save to DB

Incremental (run on every PR):
  For each changed file:
    removeFile(path) → re-parse → upsert symbols/edges → re-embed
  Only touch changed files — never re-index the whole repo
```

For 10 changed files, only 10 files worth of work regardless of repo size.

### Retriever — Graph-Augmented RAG

The retriever is what makes the reviewer system-aware:

```
diff → extract changed symbols
     → look up in dependency graph
     → BFS 1-2 hops: callers + callees
     → semantic neighbors (vector search, deduplicated against graph results)
     → serialize for prompt (signatures for distant nodes, full source for direct)
```

**70% token reduction** vs. naive "dump related files" — achieved by sending signatures + docstrings for distant neighbors and full bodies only for direct neighbors.
...
SSE (not WebSockets) for indexing progress — simpler, works through proxies, one-directional.

---

## Layer 3 — CI Adapters

Both GitHub Actions and Azure Pipelines behind one `CIAdapter` interface. The dashboard pre-fills the YAML with the repo's actual ID — users copy and paste, never manually configure.
...
These features address the root causes of why AI code review fails (based on benchmark research showing 39–49% F1 and 40% alert ignore rates). They map into the layers above.


| Priority | Feature                                           | Layer       | Impact    | Status      |
| -------- | ------------------------------------------------- | ----------- | --------- | ----------- |
| **P1**   | Graph-aware context (Tree-sitter + BFS retriever) | Layer 1     | 🔴 High   | Not Started |
| **P1**   | Precision-first output (confidence threshold)     | Layer 0 mod | 🔴 High   | Not Started |
| **P1**   | Institutional memory (team decision store)        | Layer 1/2   | 🔴 High   | Not Started |
| **P2**   | Intent / ticket verification (Jira/GitHub Issues) | Layer 2     | 🔴 High   | Not Started |
| **P2**   | Production metadata injection (git signals)       | Layer 1     | 🔴 High   | Not Started |
| **P2**   | TypeScript type diagnostics (`tsc` + Tree-sitter) | Layer 1     | 🟡 Medium | Not Started |
| **P2**   | Codebase embeddings (vector search layer)         | Layer 1     | 🔴 High   | Not Started |
| **P3**   | Cross-repository impact analysis                  | v2+         | 🔴 High   | Not Started |


### P1: Precision-First Output

Confidence threshold per comment — if nothing clears the bar, post nothing. "This PR looks good" is a valid output. Targets the alert fatigue problem.
...
- Review includes an "Intent Check" section: ✅ / ❌ / ⚠️ per criterion
...
```typescript
// Cheapest signals — zero external dependencies
{ path, commitCount30d, daysSinceLastChange, authorCount }
// → "⚠️ src/payments/charge.ts — 23 changes in 30 days, 8 authors"
```
...
Everything else — multi-repo, team management, cross-service mesh — is v2+.

---

## What NOT to Do

- Do not re-index the whole repo on every PR — only touch changed files
- Do not store raw source code — symbols, edges, embeddings only
- Do not use a graph database (Neo4j) — in-memory + SQL is sufficient
- Do not build the dashboard before the API works — test with curl first
- Do not hard-code LLM provider — configurable from day one via `LLM_PROVIDER`
- Do not make Postgres required for self-hosting — SQLite is the default
- Do not surface `graph_depth` in UI — use Fast / Standard / Deep
- Do not break Layer 0 (the existing reviewer) — everything wraps around it
