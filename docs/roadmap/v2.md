...
> Research benchmark: even the best AI code review tools score only **39–49% F1** on real PRs. Up to **40% of AI alerts are ignored** by developers once trust is lost. The items below address the root causes.

| Priority | Feature | Impact | Effort | Status |
|----------|---------|--------|--------|--------|
| **P1** | Precision-First Output (confidence threshold) | 🔴 High | 🟢 Low | Not Started |
| **P1** | Static Analysis Layer (deterministic signal-first) | 🔴 High | 🟡 Medium | Not Started |
| **P1** | Institutional Memory (team decision store) | 🔴 High | 🟡 Medium | Not Started |
| **P2** | Intent / Ticket Verification | 🔴 High | 🟡 Medium | Not Started |
| **P2** | TypeScript Type Checking | 🟡 Medium | 🟡 Medium | Not Started |
| **P2** | Codebase Embeddings | 🔴 High | 🔴 High | Not Started |
| **P2** | Production Metadata Injection | 🔴 High | 🔴 High | Not Started |
| **P3** | Multi-language LSP | 🟡 Medium | 🔴 High | Not Started |
| **P3** | Cross-Repository Impact Analysis | 🔴 High | 🔴 High | Not Started |

---

## P1: Precision-First Output

**The problem:** All current AI review tools — including CodeRabbit — default to high-recall output: comment on everything, let the developer sort it out. This creates alert fatigue. In one documented evaluation, CodeRabbit posted 10 comments on a 493-line PR — all noise, zero signal. Once trust is lost, developers dismiss all output including the real finds.
...
For each candidate comment:
  ...
  if score < THRESHOLD → discard silently
  else → post

If no comments remain → post "No significant issues found"
```

**Implementation approach:**
- Add a `confidence` field to `ReviewComment` (0.0–1.0)
...
**The problem:** LLM-only reviewers hallucinate findings and miss deterministic bugs. CodeRabbit's advantage is running 40+ SAST/linter tools and using the LLM to filter and explain their output — not to invent findings. AgnusAI currently does the opposite: pure LLM generation with no deterministic ground truth.
...
Static Analysis Layer
 ├── ESLint / Biome (JS/TS)
 ├── Semgrep rules (security patterns)
...
**Why this works:** Static analysis findings are deterministic — same input always produces the same output. This eliminates the non-determinism problem and grounds the LLM in verified signals rather than guesses.

**Implementation approach:**
- Add an optional `staticAnalysis` step in `ReviewEngine` before LLM call
- Start with TypeScript compiler (`tsc --noEmit`) — zero new dependencies
...
**The problem:** A team spends 45 minutes debating a locking strategy, reaches a decision. Next PR touching the same code gets an AI comment re-opening the same debate. AI tools have no access to prior review thread conclusions. CodeRabbit's "learning" only suppresses dismissed comment _types_ — it doesn't store reasoning or enforce specific decisions.
...
LLM enforces them — and cites the original decision
```
...
Review includes an "Intent Check" section:
  ✅ Criterion 1: Satisfied (line 42 handles this)
  ❌ Criterion 3: Not addressed — no input validation added
  ⚠️  Criterion 2: Partially — error case missing
```
...
// Injected into prompt: "⚠️ src/payments/charge.ts — 23 changes in last 30 days, 8 authors"
```
...
Dependency graph query:
  ...
  → service-b, service-c, mobile-api, admin-panel
 │
 ▼
For each dependent repo:
  Fetch usages of UserDTO.email
  Flag: "service-b/src/user.service.ts:42 — references .email which no longer exists"
 │
 ▼
Review comment on the original PR:
  ⚠️ Breaking change detected — 3 downstream consumers affected
```
...
└─────────────────────────────────────────────────────────────────────┘
     ...
     ▼                         ▼                         ▼
┌──────────────┐      ┌─────────────────────┐    ┌───────────────────┐
│ Static       │      │  Context Builder     │    │  Institutional    │
│ Analysis     │      │                      │    │  Memory Store     │
│ Layer (P1)   │      │ • Diff + file sigs   │    │                   │
│              │      │ • Ticket/intent (P2) │    │ • Team decisions  │
│ • tsc        │      │ • Production signals │    │ • Dismissed rules │
│ • ESLint     │      │ • Embeddings (P2)    │    │ • Custom rules    │
│ • Semgrep    │      │ • Type info (P2)     │    │                   │
└──────────────┘      └─────────────────────┘    └───────────────────┘
     │                         │                         │
     └─────────────────────────┼─────────────────────────┘
                               ▼
...
│   "Triage and explain static findings. Add issues static tools      │
│    cannot detect. Score confidence 0–1 for each comment."           │
└─────────────────────────────────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────────┐
│                   Precision Filter (P1)                             │
│          Drop comments below confidence threshold                   │
│          If nothing remains → post "No significant issues found"    │
└─────────────────────────────────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────────┐
│                        Comment Manager                              │
│              • Deduplication + version claim filter                 │
